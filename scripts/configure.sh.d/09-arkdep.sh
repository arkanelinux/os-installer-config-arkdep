...
if [[ $memtotal -lt 4500000 ]]; then
	...
	sudo btrfs filesystem mkswapfile --size 2G $workdir/arkdep/shared/var/swapfile \
		|| quit_on_err 'Failed to create swapfile'
elif [[ $memtotal -lt 8500000 ]]; then
	# If RAM is less than 8.5GB, create a 4GB swapfile
	sudo btrfs filesystem mkswapfile --size 4G $workdir/arkdep/shared/var/swapfile \
		|| quit_on_err 'Failed to create swapfile'
else
	# Else create a 6GB swapfile
	sudo btrfs filesystem mkswapfile --size 6G $workdir/arkdep/shared/var/swapfile \
		|| quit_on_err 'Failed to create swapfile'
fi
